...
pub fn rasterize_dab(canvas: &mut Canvas, dab: &Dab, tip: &crate::dab::Tip) {
...
pub fn rasterize_dabs(canvas: &mut Canvas, dabs: &[Dab], tip: &crate::dab::Tip) {
    for dab in dabs {
        rasterize_dab(canvas, dab, tip);
    }
}
...
mod tests {
    ...
    fn a_dab_paints_something() {
        ...
        rasterize_dab(&mut c, &black_dab(128.0, 128.0, 8.0, 0.5), &Tip::default());
    ...
    fn a_zero_radius_dab_paints_nothing() {
        let mut c = Canvas::new(256, 256);
        rasterize_dab(&mut c, &black_dab(128.0, 128.0, 0.0, 0.5), &Tip::default());
    ...
    fn a_fully_transparent_dab_paints_nothing() {
        ...
        rasterize_dab(&mut c, &d, &Tip::default());
        assert_eq!(c.tiles().count(), 0);
    }
    ...
    fn a_hard_dab_center_is_fully_opaque_paint() {
        let mut c = Canvas::new(256, 256);
        rasterize_dab(&mut c, &black_dab(100.5, 100.5, 8.0, 0.0), &Tip::default());
    ...
    fn dabs_off_canvas_are_clipped_not_panicking() {
        ...
        rasterize_dab(&mut c, &black_dab(-50.0, -50.0, 8.0, 0.5), &Tip::default());
        assert_eq!(c.tiles().count(), 0);
        // Straddling the origin should paint only the in-bounds part.
        rasterize_dab(&mut c, &black_dab(0.0, 0.0, 8.0, 0.5), &Tip::default());
        assert_eq!(c.tiles().count(), 1);
    }
    ...
    fn batch_rasterization_matches_one_at_a_time() {
        ...
        rasterize_dabs(&mut batched, &dabs, &Tip::default());

        let mut individually = Canvas::new(256, 256);
        for d in &dabs {
            rasterize_dab(&mut individually, d, &Tip::default());
        }
